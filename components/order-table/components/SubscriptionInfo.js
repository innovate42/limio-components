...
const { addresses: customerAddress} = useLimioUserSubscriptionAddresses(id)
      ...
      <div>
        ...
        <tr className="dark:text-white text-left flex flex-row md:flex-col ">
              ...
              {isDelivery && 
                ...
                <div className="mb-2" onMouseOver={() => setShowFullAddress(true)} onMouseOut={() => setShowFullAddress(false)} onTouchStart={() => setShowFullAddress(true)} onTouchEnd={() => setShowFullAddress(false)}>
                  {!showFullAddress ?
                  <td className="px-4 py-2  text-sm">{`${addresses.deliveryAddress.address1}...`}</td>
                  :
